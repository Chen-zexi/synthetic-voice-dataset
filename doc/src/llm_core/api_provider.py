...
class LLM:
    ...
    def get_llm(self):
        ...
        if self.provider == "openai":
            ...
            # Note: Reasoning parameters are not supported in current OpenAI SDK (v1.86.0)
            # Create ChatOpenAI with supported parameters only
    ...
    def _prepare_openai_params(self) -> Dict[str, Any]:
        ...
        # Note: Response API features are handled through stream_usage parameter
        # The output_version parameter is not supported in OpenAI SDK v1.86.0
        ...
        # Note: Reasoning models (gpt-5, etc.) are not yet available in OpenAI API
        # For now, apply standard parameters to all models
        ...
        elif "max_completion_tokens" in self.model_parameters:
